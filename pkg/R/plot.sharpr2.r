...
plot.sharpr2 <- function(re, tr, unc = 'CI', loess=FALSE, add=FALSE, xlab='Position', ylab='Regulatory Score', cicol = "orange", cimcol = 'grey', sreg = TRUE, ...)
{
	if(is.na(tr))
	{
	...
	region <- as.numeric(unlist(strsplit(re$region[[tr]],'-')))
	
	if(is.na(re$score[[tr]]$est_a[1]))
	{
		stop("There is no result for this tiled region.")
	}
	
	if(var(re$score[[tr]]$est_a)==0)
	{
		if(add==FALSE)
		{
			plot.default(region[1]:region[2],re$score[[tr]]$est_a,type='l',xlab=xlab, ylab=ylab, ...)
		}else{
			lines(region[1]:region[2],re$score[[tr]]$est_a,xlab=xlab, ylab=ylab, ...)
		}
		...
		{	
			if(add==FALSE)
			{
				plot.default(loess.smooth(region[1]:region[2],re$score[[tr]]$est_a,degree = 2),type='l',xlab=xlab, ylab=ylab, ...)
			}else{
				lines(loess.smooth(region[1]:region[2],re$score[[tr]]$est_a,degree = 2),xlab=xlab, ylab=ylab, ...)
			}
		}else{
			if(add==FALSE)
			{
				if(is.na(re$score[[tr]]$var_nb[1]))
				{
					plot.default(region[1]:region[2],re$score[[tr]]$est_a,type='l',xlab=xlab, ylab=ylab, ...)
				}else{
					if(unc=='MSE')
					{
						if(!is.na(re$score[[tr]]$mse[1]))
						{
							lower <- re$score[[tr]]$est_a - re$score[[tr]]$mse
							upper <- re$score[[tr]]$est_a + re$score[[tr]]$mse
						}else{
						...
						if(unc=='CI')
						{
							lower <- re$score[[tr]]$est_a - 1.96*sqrt(re$score[[tr]]$var_nb)
							upper <- re$score[[tr]]$est_a + 1.96*sqrt(re$score[[tr]]$var_nb)
						}	
					}		
					
					plot.default(region[1]:region[2],re$score[[tr]]$est_a,type='l', ylim=c(min(lower),max(upper)), xlab=xlab, ylab=ylab, ...)
					...
					if(!is.na(re$sig_reg[[tr]][1]))
					{
						if(sreg==TRUE)
						{
							points(x=re$sig_reg[[tr]], y=rep(0,length(re$sig_reg[[tr]])),col='red',cex=1,pch=20)
							n_sreg <- split(re$sig_reg[[tr]], cumsum(c(1, diff(re$sig_reg[[tr]]) != 1)))
							...
							polygon(c(re$motif[[tr]],rev(re$motif[[tr]])), c(min(lower),min(lower),max(upper),max(upper)), col=rgb(0, 0, 1,0.2), border=NA)
							# lines(region[1]:region[2],re$cutoff[[tr]]*sqrt(re$score[[tr]]$var_nb), col=rgb(0,1,0,alpha=0.5))
						}
					}
				}
				...
				if(is.na(re$score[[tr]]$var_nb[1]))
				{
					...
					lines(region[1]:region[2],re$score[[tr]]$est_a,type='l', ylim=c(min(lower),max(upper)), xlab=xlab, ylab=ylab, ...)
